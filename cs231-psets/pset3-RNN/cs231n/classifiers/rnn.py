...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (4)
        drnn_or_lstm_out, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dtemporal_affine_out,
                                                                                        temporal_affine_cache)
        # (3)
        if self.cell_type == 'rnn':
            dword_embedding_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(drnn_or_lstm_out,
                                                                                                rnn_cache)
        elif self.cell_type == 'lstm':
            dword_embedding_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(drnn_or_lstm_out,
                                                                                                 lstm_cache)
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)
        # (2)
